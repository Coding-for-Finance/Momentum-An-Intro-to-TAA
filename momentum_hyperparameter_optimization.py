...
lookback_holding_combination = list(itertools.product(list_lookback, list_holding_period))
dict_hyperpar_output = {'combination': [], 'summary stats':[], 'geometric average best strat':[],'best strat name by return':[], 'sharpe ratio best strat':[], 'best strat name by sharpe' : []}

for lookback, holding_period in lookback_holding_combination:
    ...
    dict_hyperpar_output['combination'].append(str(lookback) + ','+ str(holding_period))
    dict_hyperpar_output['summary stats'].append(df_summary)
    dict_hyperpar_output['geometric average best strat'].append(max_return)
    dict_hyperpar_output['best strat name by return'].append(max_return_name)
    dict_hyperpar_output['sharpe ratio best strat'].append(max_return)
    dict_hyperpar_output['best strat name by sharpe'].append(max_sharpe_name)

    # %% Visualizations

    ## NAV Plot
    df_NAVs = summary.NAVs # saving a df of the NAVs we calculate in the class "port_an"
    df_NAVs.plot() # calling a simple plot
    plt.title(f"NAV Comparison of ({lookback},{holding_period}) momentum strategies") # adding a title with a dynamic variable if one decides to add/reduce strategies
    plt.xlabel('Date') # adding x and y-axes labels
    plt.ylabel('Cumulative NAVs')
    plt.savefig(f"plots/({lookback},{holding_period})NAVs.png")
    ...
    for strategy, name in zip(dict_scaled_weights, strategy_names):
        ...
        plt.title(f"Weights of ({lookback},{holding_period}) {name} strategy")
        plt.savefig(f"plots/({lookback},{holding_period}) {name}_weights.png")
        plt.show()
    ...
    for strategy, name in zip(dict_portfolio_returns, strategy_names):
        sns.regplot(x = dict_portfolio_returns[strategy], y = df_BM)
        plt.title(f"Scatter plot of ({lookback},{holding_period}) {name} strategy against benchmark")
        # create a folder called "plots" in the working directory for saving the plots
        plt.savefig(f"plots/({lookback},{holding_period}){name}_scatter.png")
        plt.show()
